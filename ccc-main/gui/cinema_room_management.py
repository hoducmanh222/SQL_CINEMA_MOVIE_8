...
class CinemaRoomManagementFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Store seat buttons for preview
        self.seat_buttons = {}
    ...
    def create_widgets(self):
        ...
        # Add seat layout preview frame
        self.seat_preview_frame = tk.LabelFrame(details_frame, text="Seat Layout Preview")
        self.seat_preview_frame.grid(row=4, column=0, columnspan=2, padx=5, pady=10, sticky="nsew")
        # Make sure the preview expands
    ...
    def on_room_select(self, event):
        ...
        if room:
            ...
            # Show seat layout preview
            self.show_seat_layout_preview(room["Capacity"])
    ...
    def clear_form(self):
        ...
        # Clear seat preview
        for widget in self.seat_preview_frame.winfo_children():
            widget.destroy()
        self.seat_buttons = {}
    ...
    def show_seat_layout_preview(self, capacity):
        """Display a seat grid preview for the given capacity (all seats unbooked/green)"""
        # Clear previous preview
        for widget in self.seat_preview_frame.winfo_children():
            widget.destroy()
        self.seat_buttons = {}

        # Determine rows/columns (try to use 10 seats per row, like ticket booking)
        columns = 10
        rows = (capacity + columns - 1) // columns
        row_labels = [chr(ord('A') + i) for i in range(rows)]

        # Screen label
        screen_label = tk.Label(self.seat_preview_frame, text="SCREEN", bg="lightgray", width=30)
        screen_label.grid(row=0, column=0, columnspan=columns+1, padx=5, pady=5)

        seat_num = 1
        for r, row in enumerate(row_labels):
            tk.Label(self.seat_preview_frame, text=row).grid(row=r+1, column=0, padx=2)
            for col in range(1, columns+1):
                if seat_num > capacity:
                    break
                seat_id = f"{row}{col}"
                btn = tk.Button(self.seat_preview_frame, text=seat_id, width=3, height=1,
